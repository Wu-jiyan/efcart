...
<script>
function updateMobileBottomTotalWidth() {
  const verticalMenu = document.querySelector('.simplebar-offset');
  const mobileBottomTotal = document.querySelector('.mobile-bottom-total');
  if (verticalMenu && mobileBottomTotal) {
    const verticalMenuWidth = verticalMenu.offsetWidth;
    const windowWidth = window.innerWidth;
    mobileBottomTotal.style.width = `${windowWidth - verticalMenuWidth}px`;
  }
}
  ...
  const menuObserver = new MutationObserver(mutations => {
    mutations.forEach(mutation => {
      if (mutation.type === 'attributes' && mutation.attributeName === 'style') {
        updateMobileBottomTotalWidth();
      }
    });
  });

  const menuTargetNode = document.querySelector('.simplebar-offset');
  const menuConfig = { attributes: true, attributeFilter: ['style'] };

  menuObserver.observe(menuTargetNode, menuConfig);
})();

updateMobileBottomTotalWidth();

window.addEventListener('resize', updateMobileBottomTotalWidth);
</script>
...
<div class="mobile-bottom-total">
 <span class="total" style="margin-left: 25px;color: #0064ff">{$Lang.total_price}：{if !$ConfigureTotal.type}
{$ConfigureTotal.currency.prefix}{:bcadd($ConfigureTotal.signal_price,$ConfigureTotal.signal_setupfee)}
{else}
{$ConfigureTotal.currency.prefix}{:bcadd($ConfigureTotal.sale_signal_price,$ConfigureTotal.sale_setupfee_total)}
{/if}</span>
 <button type="button" style="position: fixed;right: 25px;bottom: calc(2rem - 19px);cursor: pointer;border-radius: 0px;" class="btn btn-primary" id="addToCartBtnTwo"><i class="mdi mdi-cart-arrow-right mr-1"></i>{$Lang.add_cart}</button>
</div>
<script>
$(function() {
